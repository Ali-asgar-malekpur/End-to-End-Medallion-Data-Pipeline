...
dbutils.widgets.text("data_source", "orders_2", "Data Source")

catalog = dbutils.widgets.get("catalog")
data_source = dbutils.widgets.get("data_source")

base_path = f'r2://sportsbar-dp@3be93c912191c1bf8cf7912818757bf4.r2.cloudflarestorage.com/{data_source}'
landing_path = f"{base_path}/landing/"
processed_path = f"{base_path}/processed_csv_files/"
...
for file_info in files:
    # 1. Skip the destination folder itself
    # 2. Skip any other sub-directories (isDir check)
    if not file_info.isDir() and "processed_csv_files" not in file_info.path:
        
        dbutils.fs.mv(
            file_info.path, 
            f"{processed_path}{file_info.name}", # Ensure processed_path ends with /
            True
        )
